...
public partial class EnemySpawnSystem : SystemBase
{
    ...
    protected override void OnUpdate()
    {
        ...
        {
            prefab = GetSingleton<EnemySpawnPropertiesComponent>().EnemyPrefab;
            if (prefab == Entity.Null) return;
        }

        Entity settingsEntity = GetSingletonEntity<EnemySpawnPropertiesComponent>();
        ...
        .WithCode(() => {
            for (int i = count; i < settings.NumberOfEnemies; ++i)
            {
                ...
                VelocityPropertiesComponent velocity = new VelocityPropertiesComponent();
