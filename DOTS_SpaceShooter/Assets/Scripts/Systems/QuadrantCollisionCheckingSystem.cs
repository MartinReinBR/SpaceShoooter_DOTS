...
public partial class QuadrantCollisionCheckingSystem : SystemBase
{
    ...
    protected override void OnUpdate()
    {
        ...
        Entities
           ...
           .ForEach((Entity projectile, in Translation projectilePos, in CollisionSphereComponent sphere) =>
           {
               ...
               if (quadrantMap.TryGetFirstValue(mapKey, out enemy, out mapIterator))
               {
                   do
                   {
                       CollisionAABBComponent aabb = entityManager.GetComponentData<CollisionAABBComponent>(enemy);
        ...
        Entities
           .WithAll<PlayerTag>()
           .WithStructuralChanges()
           .ForEach((Entity player, PlayerHealth playerHealth, Transform playerTransform, in CollisionAABBComponent playerCollision) =>
           {
               ...
               if (quadrantMap.TryGetFirstValue(mapKey, out enemy, out mapIterator))
               {
                   do
                   {
                       CollisionAABBComponent enemycollision = entityManager.GetComponentData<CollisionAABBComponent>(enemy);
    ...
    private bool IsAabbCollision(CollisionAABBComponent a, CollisionAABBComponent b)
    {
        return (
             a.XMin < b.XMax &&
             a.XMax > b.XMin &&
    ...
    private bool IsSphereAABBCollision(CollisionSphereComponent sphere, CollisionAABBComponent aabb)
    {
    ...
    private void ResetEnemy(Entity enemy)
    {
        ...
        EnemySpawnPropertiesComponent settings = GetSingleton<EnemySpawnPropertiesComponent>();
